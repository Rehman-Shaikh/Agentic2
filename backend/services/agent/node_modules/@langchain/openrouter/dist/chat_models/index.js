...
var ChatOpenRouter = class extends BaseChatModel {
	...
	*/
	...
	constructor(modelOrFields, fieldsArg) {
		...
		super(fields);
		this._addVersion("@langchain/openrouter", "0.4.12");
	...
	*/
	async _generate(messages, options, runManager) {
		...
		const data = await (await this.caller.callWithOptions({ signal: options.signal }, async () => {
			...
			if (!nextResponse.ok) throw await OpenRouterError.fromResponse(nextResponse);
			return nextResponse;
		})).json();
	...
	async *_streamChatModelEvents(messages, options, _runManager) {
		...
		yield* convertOpenRouterStream(readChunks(), { streamUsage: shouldStreamUsage });
	}
	/**
