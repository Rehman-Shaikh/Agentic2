	...
	*/
	async _generate(messages, options, runManager) {
		...
		const data = await (await this.caller.callWithOptions({ signal: options.signal }, async () => {
			...
			if (!nextResponse.ok) throw await require_errors.OpenRouterError.fromResponse(nextResponse);
			return nextResponse;
		})).json();
	...
	async *_streamChatModelEvents(messages, options, _runManager) {
		...
		yield* require_stream_events.convertOpenRouterStream(readChunks(), { streamUsage: shouldStreamUsage });
	}
	/**
